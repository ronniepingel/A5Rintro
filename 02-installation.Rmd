...
1. Ladda ner den senaste versionen från https://cran.r-project.org/bin/macosx/.Notera att versionen
...
2. Klicka på ''Download R (versionnummer) for Windows''. Det kan vara så att den senaste versionen inte fungerar på ditt Windows. Prova då en tidigare version. Ditt versionnummer kan även skilja sig åt från den som används i detta exempel eftersom det kan ha kommit senare versioner, men för din del saknar just detta praktisk betydelse.
...
Om du mot förmodan inte skulle lyckas installera R och RStudio, så finns möjligheten att koppla upp sig till Statistiska institutionsn datorer via fjärrskrivbordet och på så sätt få tillgång nödvändig programvara.
