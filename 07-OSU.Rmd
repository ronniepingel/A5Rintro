...
I ett nybyggt höghusområde önskade kommunen undersöka förskolebehovet. Kommunen gjorde därför ett obundet slumpmässigt urval utan återläggning (OSU-UÅ)
av 100 hushåll. Frågan som ställdes var hur många platser som hushållen är i behov av. Totalt finns det 800 hushåll i området. Av de tillfrågade hushållen angav 56 inget behov, 34 hade behov av en plats, 7 hade behov av två platser och 3 hade behov av tre platser.
...
# Konfidensintervall
 alpha <- 0.05
 z_alpha <- qnorm((1 - alpha/2)) # 'qnorm' ger oss i detta fall den 97.5:e kvantilen från en normalfördelning med medelvärde 0 och standardavvikelse 1. Man kan ändra värdet som är kopplat till kommandot för att få andra kvantiler så att beräkningarna passar den konfidensgrad vi vill ha.
 ll_tau <- tauhat - z_alpha*sqrt(vhattauhat)
 ul_tau <- tauhat + z_alpha*sqrt(vhattauhat) 

```

* Svar: Med 95\% säkerhet täcker intervallet $345 < \tau < 567$ det totala antalet önskade förskoleplatser i området.

```{r, echo=TRUE, eval=TRUE, collapse=TRUE, prompt=FALSE, comment=''} 
# b)
 x <- sum(platsbehov == 0) # Antal händelser
 phat<- x/n    # stickprovsandelen nollor blir vår punktskattning.
...
# Kan vi lita på intervallet? Kolla CGS
 n*phat*(1-phat)
```
* Svar: Med 95\% säkerhet täcker intervallet $0,469 < p < 0,651$ andelen i området som inte önskar någon förskoleplats.
...
# Skapa data enligt uppgift. 
 kommunprodukt <- c(rep("U", times = 60),
                    rep("T", times = 100),
                    rep("D", times = 40))
 N <- 8000
 n <-  200
 
 kommunprodukt_bin <- NA
 kommunprodukt_bin[kommunprodukt == "U"] <- 0
 kommunprodukt_bin[kommunprodukt == "T"] <- 0
 kommunprodukt_bin[kommunprodukt == "D"] <- 1
 

 x <- sum(kommunprodukt_bin)
 phat <- x/n
 tauhat <- N*phat
 vhatphat <- (1 - n/N)*phat*(1 - phat)/( n - 1)
...
```
* Svar: Medelfelet (standardfelet, standard error) är 0,032 vilket är den skattade standardavvikelsen i stickprovsandelens samplingfördelning.
...
```{r}
 N <- 2000
 2*z_alpha*sqrt(((N - n)/(N-1))*(p*(1 - p))/n)
```
...
(Notera att $\sigma^2_x$ och $s^2_x$ ofta skrivs utan index, samt att $V(\bar{x})$ och $\hat{V}(\bar{x})$ alternativt kan skrivas $\sigma^2_{\bar{x}}$ och $\hat{\sigma}^2_{\bar{x}}$.) + h) Om $n$ är stort och vi kan använda tumregeln $n > 30$ (som måste användas med viss försiktighet), vilken varians och approximativ fördelning har $\bar{x}$? 
...
a) 
* Mål: Bestämma nödvändig stickprovsstorlek $n$ om längden för ett 95\% konfidensintervall för medelutgiften inte får överstiga 50 kronor. Vi utgår från att vi vet populationsstandardavvikelsen (250) varpå $V(\bar{x})=\left(\frac{N-n}{N-1}\right)\frac{\sigma^2}{n}$. 

* Beräkningar: vi kan lösa uppgiften algebraiskt. Längden för ett 95\% konfidensintervall kan skrivas som
