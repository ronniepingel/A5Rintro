...
+ Använd `r # ` används för att kommentera kod. *Kommentera alltid kod!*. Vid all programmering är det viktigt att förklara koden så att du själv och andra snabbt förstår vad koden gör.  Det är viktigt att få rutin på att kommentera, så kommentera även enkel kod.
</div>
...
w <- x + (y^2 - u)/v
w
# Addera x vektor med en vektor med 3 element
x + c(2,5,6)
...
Resultatet är alltså att om en vektorn $x$ adderas till en vektor bestående av 3 element, dvs antalet element skiljer sig åt, så erhålls ett felmeddelande. Det gäller oavsett räknesätt. Om däremot den ena vektorn är en konstant, som i exemplet med vektorn $u$ ovan, så fungerar beräkningarna genom att konstanten beräknas för alla element i den större vektorn.

Vi ska nu studera hur vektorer, och följaktligen data, kan hanteras. I praktiken måste data anpassas och manipuleras för planerade analyser. Det mest grundläggande i denna process  är att konstatera att varje element har en position. Med hakparenteser `[]` erhålls åtkomst till ett eller flera element,  antingen genom att direkt identifiera positionen i vektorn med en siffra eller med **logiska operatorer**. Några vanliga och viktiga logiska operatorer är:
...
8. Fortsätt scriptet genom att skriva in nedanstående kod. Spara och kör koden.


```{r, eval=FALSE}
# Visa element nummer 2 i vektorn x 
x[2]
# Visa med negativ indexering alla element i x men inte element 2
x[-2]
...
Du ser nu att med hjälp av indexering på ett enkelt sätt kan välja element i vektorer. Du ska nu med hjälp av indexering
förändra värd

+ skapa en ny vektor som en är delmängd av den ursprungliga vektorn.
+ förändra element i en numerisk vektor
+ förändra element i en character-vektor.

9. Skriv in nedanstående kod och spara scriptet. Kör sedan scriptet.
...
ysub[ysub >= 6] <- 0
# Sätt värdet 99 i vektorn ysub på positionen där nordic_countries är "Finland"
...
Ovanstående sätt att skapa nya vektorer och förändra element används **mycket ofta** i R. 
...
Precis som vad gäller vektorer går det att utföra elementvisa beräkningar. Det kräver att matriserna har samma antal rader och kolumner. Det går även att utföra räkneoperationer med en konstant. Observera att elementvis multiplikation **inte** är samma som det som betecknas **matrismultiplikation**. Det är naturligtvis inga problem att i R använda matrismultiplikation, men detta går utöver kursens mål.
...
Det går att tillämpa logiska operatorer även på matriser, men den extra dimensionen gör detta omständligt. Dessutom konstaterade vi att matrisern endast kan hantera en datatyp. Därför introduceras en tredje typ av dataobjekt som är enklare att arbeta med.


### Data frames
En **data frame** är i praktiken det vanligaste objektet för dataanlys. Liksom matrisen har en data frame två dimensioner (rader och kolumner), men kan till skillnad från matrisen innehålla element med olika datatyper, t ex både numeriska värden och bokstäver. Vidare innehåller en data frame detaljerad information om exempelvis **variabelnamn** och **variabeltyp**. 
...
Man alltså använd enbart hakparentes, men oftast använder man vid datahantering `$` eftersom det är transparent. 
...
Det är ingen mening med att lista alla viktiga funktioner, utan på denna kurs kommer funktioner att introduceras efterhand.
Men några funktioner som är är bra att känna till och som 
...
Var försiktig med denna funktion eftersom alla objekt i minnet försvinner! Emellertid är god idé inleda en dataanlys med denna kod för att säkerställa att  minnet är tomt innan data läses in och analyser görs så att konflkter mellan objekt inte uppstår.Förslagsvis används denna funktion inför varje ny övningsuppgift, såvida inte uppgiftern bygger på information från tidigare uppgifter.
